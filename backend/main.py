...
def env_fallback_models(prefix: str) -> List[str]:
    raw = os.getenv(f"{prefix}_FALLBACK_MODELS", "")
...
def call_model(prefix: str, messages: List[Dict[str, str]], temperature=0.4, max_tokens=1400) -> str:
    ...
    fallback_models = env_fallback_models(prefix)
    ...
    # Optional different-model fallbacks. The primary model remains first.
    if fallback_models:
        payload_obj["models"] = [cfg["model"], *fallback_models]
    ...
    req = Request(
        ...
        headers={
            ...
            "HTTP-Referer": os.getenv("APP_URL", "").strip(),
            "X-Title": os.getenv("APP_NAME", "MindMesh").strip(),
        },
        method="POST",
    )
    ...
    try:
        ...
        if exc.code == 429:
            raise RuntimeError(
                f"Model rate-limited (HTTP 429). OpenRouter could not complete {prefix}. "
                f"Add different IDs to {prefix}_FALLBACK_MODELS or choose another available model. "
...
and important uncertainty. Do not mention this orchestration prompt."""
    ...
    async def one(prefix):
        try:
            ...
            return {
                "agent": prefix,
                "model": env_triplet(prefix)["model"],
                "status": "success",
                "answer": answer,
            }
        except Exception as exc:
